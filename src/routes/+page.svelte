<script lang="ts">
  ...
  // Maps browser KeyboardEvent.code → rdev Key debug string.
  // This avoids the rdev→IPC round-trip for capturing keys in the UI,
  // which was unreliable for Alt, Space, and other special keys.
  const CODE_TO_RDEV: Record<string, string> = {
    ...
    Digit0: 'Num0',
    // Letters (same in browser and rdev)
  ...
  function captureKey(field: string) {
    ...
    isCapturing = field;
    // Blur any focused element so Space/Enter can't re-click the button
  ...
  function handleWindowKeydown(e: KeyboardEvent) {
    ...
    // Block all browser/WebView defaults while capturing
    // (prevents Space from clicking focused button, Alt from opening menu, etc.)
    e.preventDefault();
  ...
  // Prevent keyup defaults too — important for Alt (avoids SC_KEYMENU on Windows)
